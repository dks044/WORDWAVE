...
function GrammarBook({ grammarBook }) {
  ...
  if (!grammarBook) return null; 
  return (
    <>
      <Title>{grammarBook.name}</Title>
      <hr />
      <div ref={ref}>
        {grammarBook.categories && grammarBook.categories.map((category, index) => (
          <div key={index} style={{ width: '100%' }}>
            ...
            <Modal.Body>
              ...
              ✌️ '영어 문장 객관식 퀴즈' 와 '영어 문장 퍼즐 퀴즈' 로 총 2종류가 구현되있으며, 랜덤으로 출제 됩니다.<br/><br/>
              </h5>
            </Modal.Body>
            <Modal.Footer>
              ...
              <Button onClick={() => navigate(`/grammarBooks/${grammarBook.id}/${selectedCategory}`)} variant="primary">
                공부하기!
              </Button>
            </Modal.Footer>
          </Modal>

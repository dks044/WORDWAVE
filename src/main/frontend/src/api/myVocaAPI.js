...
export async function getMyVocaBooksAPI({userId}){
  const response =  await api.get(`/api/myvocabook/myVocaBookList/${userId}`);
...
export async function createMyVocaBookAPI(name, userId, imageFile) {
  ...
  return await api.post(`/api/myvocabook/create`, formData, {
      headers: {
...
export async function getMyVocaBookAPI({myVocaBookId,userId}){
  const response = await api.get(`/api/myvocabook/${myVocaBookId}/${userId}`)
  return response.data;
}

//myVocaBook의 정보(업데이트폼 활용)
export async function getOneMyVocaBookAPI({myVocaBookId,userId}){
  const response = await api.get(`/api/myvocabook/get/${myVocaBookId}/${userId}`)
  return response.data;
}

//MyVocaBook 삭제
export async function deleteMyVocaBookAPI({myVocaBookId}){
  const response = await api.delete(`/api/myvocabook/delete/${myVocaBookId}`)
  return response.data;
}
...
export async function updateMyVocaBookAPI(name, myVocaBookId, userId, imageFile) {
  ...
  return await api.patch(`/api/myvocabook/patch/myVocaBook`, formData, {
    headers: {
      'Content-Type': 'multipart/form-data'
    }
  });
...
export async function validateMyVocaBookAPI({myVocaBookId,userId}){
  const response = await api.get(`/api/myvocabook/validate/${myVocaBookId}/${userId}`)
  return response.data;
}

//MyVoca 업로드 (나만의 단어 생성)
export async function createMyVocaAPI(myVocaBookId, userId,category,myVocas){
  const response = await api.post(`/api/myvoca/create`,{myVocaBookId,userId,category,myVocas});
  return response.data;
}

//MyVcoa 업로드폼 데이터 (업로드폼에 표시될 데이터)
export async function updateFormMyVocaAPI({myVocaBookId,category}){
  const response = await api.get(`/api/myvoca/updateForm/${myVocaBookId}/${category}`)
  return response.data;
}

//MyVoca 업데이트 (나만의 단어 수정)
export async function updateMyVocaAPI(myVocaBookId,userId,prevCategory,nextCategory,myVocas){
  const response = await api.patch(`/api/myvoca/patch`,{myVocaBookId,userId,prevCategory,nextCategory,myVocas});
  return response.data;
}

//MyVoca 삭제 (나만의 단어 삭제)
export async function deleteMyVocaAPI({myVocaBookId,userId,category}){
  const response = await api.delete(`/api/myvoca/delete/${myVocaBookId}/${userId}/${category}`);
  return response.data;
}

//나만의 단어 퀴즈
export async function selectMyVocaAPI({myVocaBookId,userId,category}){
  const response = await api.get(`/api/myvoca/${myVocaBookId}/${userId}/${category}`);
  return response.data;
}